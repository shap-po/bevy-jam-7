...
use rand::Rng;
use crate::{AppSystems, PausableSystems, audio::Sfxlib, input_manager::Action, screens::Screen, theme::widget};
...
fn navigate(
    ...
    if let Some(new_room) = direction {
        ...
        if (new_room == Room::Bedroom && curr_room != Room::BedroomBed) || (curr_room == Room::Bedroom && new_room != Room::BedroomBed){
            commands.play_volume_sfx(sfx_asset.rand_door_open_and_close(), 0.05);
        }
        if new_room == Room::KitchenFridge && curr_room == Room::Kitchen{ //Kitchen -> Fridge
            commands.play_volume_sfx(sfx_asset.rand_fridge_open(), 0.5);
        }
        if new_room == Room::Kitchen && curr_room == Room::KitchenFridge{ //Fridge -> Kitchen
